...
function scene:create( event )
	...
	local minibackgroundMusic = audio.loadStream( "music/bad.mp3" )
	local minibackgroundMusicChannel = audio.play( minibackgroundMusic, { loops=-1 }  ) 

	print(minibackgroundMusicChannel)

	-- if audio.pause 일때 
	--audio.resume( minibackgroundMusicChannel )
...
-- 게임설명창
	...
	titleMsg = display.newImage(title1Group,'images/titleMsg.png')
...
local playBtn = display.newImage('images/playBtn.png', display.contentCenterX, display.contentCenterY+210)
	...
	local function playing(event)
		composer.removeScene("ministory1")
		composer.gotoScene("subGame1")
	end
